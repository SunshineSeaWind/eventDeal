...
const int iterationCount = 500; // Iteration count can be modified in need
...
void save(std::string positionFileName, std::vector<Body> *bodies) {
    std::stringstream ss;
    
    ss << "outputs/" << positionFileName << ".bin";
    std::ofstream outfile (ss.str(), std::ofstream::binary);
...
int main(int argc, const char * argv[]) {
    ...
    for (int i = startFrom; i < iterationCount; ++i) {
        cout << "Step " << i << endl;
